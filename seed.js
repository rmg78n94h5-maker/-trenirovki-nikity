(() => {
  ...
  const exercises = [
    ...
    ex('db-shoulder-press', 'Жим гантелей сидя', 'Плечи / трицепс', 'Гантели + стул', { sets: 3, repsMin: 8, repsMax: 12, restSec: 90 }, { replacements: ['machine-chest-press', 'pike-pushups'], safety: 'При плохой устойчивости выполнять только сидя с устойчивой опорой.' }),
    ...
    ex('one-arm-row', 'Тяга гантели одной рукой', 'Спина / бицепс', 'Гантель + стул', { sets: 4, repsMin: 8, repsMax: 12, restSec: 75 }, { replacements: ['seated-row-machine', 'barbell-row'], safety: 'Опирайся на стул; при плохой устойчивости не выполнять без устойчивой опоры.' }),
    ...
    ex('farmer-hold', 'Удержание гантелей', 'Хват / предплечья / кор', 'Гантели', { sets: 3, unit: 'seconds', durationSec: 35, restSec: 60 }, { replacements: ['suitcase-hold'], safety: 'При плохой устойчивости выполнять стоя у опоры или заменить.' }),
    ...
    ex('bulgarian-split-squat', 'Болгарские выпады', 'Ноги / ягодицы', 'Гантели + стул', { sets: 3, repsMin: 8, repsMax: 12, restSec: 75 }, { replacements: ['reverse-lunge', 'chair-squat'], safety: 'При плохой устойчивости заменить на присед до стула или выпады с опорой.' }),
    ...
    ex('bench-step-up', 'Зашагивания на скамью', 'Ноги / ягодицы', 'Гантели + скамья', { sets: 3, repsMin: 8, repsMax: 12, restSec: 75 }, { replacements: ['reverse-lunge', 'bulgarian-split-squat'], safety: 'Только на устойчивую скамью подходящей высоты; при плохой устойчивости не выполнять.' }),
    ...
    ex('mountain-climber-handles', 'Альпинист на упорах', 'Пресс / плечи / кардио', 'Упоры для отжиманий + коврик', { sets: 3, unit: 'seconds', durationSec: 30, restSec: 45 }, { replacements: ['dead-bug', 'stepper-short'], safety: 'При плохой устойчивости, боли в запястьях или пояснице заменить на dead bug.' }),
  ...
  window.NIKITA_SEED = {
    ...
    profile: {
      ...
      constraints: [
        'Высокая бытовая или рабочая активность в течение дня',
        'При плохой устойчивости убирать упражнения стоя с тяжёлым весом',
